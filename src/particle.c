...
void particles_emit_smoke(Point3 emitter) {
    ...
    p.pos = emitter;
    p.pos.z += 0.05;
    p.vel = (Point3){.x = rand_perturb(0, 0.0001), .y = rand_perturb(0, 0.001), .z = rand_perturb(0.0001, 0.00001)};
    ...
    p.color_end = p.color = p.color_start = COLOR_WHITE;
    p.color_end = (SDL_Color){ 0, 0, 0, 0 };
